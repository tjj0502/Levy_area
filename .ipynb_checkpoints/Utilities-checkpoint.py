...
def add_constant_term(tensor, one = 1):
    if one:
        return torch.cat([torch.tensor(torch.ones(tensor.shape[0],1).to(tensor.device)), tensor], 1)
    else:
        return torch.cat([torch.tensor(torch.zeros(tensor.shape[0],1).to(tensor.device)), tensor], 1)

def Am(dim, level):
    """
    Calculates the cardinal of the set Am stated in Tchakaloff's theorem -- Cubature formula
    """
    res = 0
    for k in range(level,0,-1):
        possible_zeros = min(level - k, k)
        for i in range(possible_zeros+1):
            coeff = math.comb(k, i)
            temp = coeff * (dim)**(k-i)
            res += temp
    return res

def graded_norm(signature: list):
    """
    Weighted norm, calculate the L2 norm of each level, then divide by the factorial
    """
    res = torch.zeros(signature[0].shape[0]).cuda()
    for ilevel in range(len(signature)):
        res += torch.linalg.norm(signature[ilevel], ord=2, axis=1)/math.factorial(ilevel)
    return res


def lead_lag_matching_coeffs(dim, level):
    '''
    Find the coordinates that should be match if lead-lag augmentation is applied
    '''
    aug_dim = 2*dim
    ori_coord = Utilities.algebra_coordinate_helper(dim, level)[1:]
    adj_coord = [np.array(label)+dim for label in ori_coord]
#     print(ori_coord, adj_coord)
    ori_pos = [Utilities.algebra_coordinate_calculator(label, aug_dim) for label in ori_coord]
    adj_pos = [Utilities.algebra_coordinate_calculator(label, aug_dim) for label in adj_coord]
    return ori_pos, adj_pos
...
if __name__ == '__main__':
    ...
    # one_tensor = torch.zeros(1, 40).to(device)
    # one_tensor[0][0] = 1
    # one_tensor = inv_transformer(one_tensor, dim, level)
    # rand_tensor = torch.rand(1, 40).to(device)
    # rand_tensor[0][0] = 1
    # rand_tensor = inv_transformer(rand_tensor, dim, level)
    #
    # print(one_tensor)
    # print(rand_tensor)
    #
    # print(mult(one_tensor, rand_tensor))

    dim = 3
    level = 3
    device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
    torch.manual_seed(52)
    rand_tensor1 = torch.rand(1, 40).to(device)
    rand_tensor1_cons = rand_tensor1.clone()
    rand_tensor1_cons[0][0] = 0
    rand_tensor1_cons = structure_checker(rand_tensor1_cons, dim, level)
